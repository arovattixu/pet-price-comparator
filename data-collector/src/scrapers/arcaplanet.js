...
/**
 * Imposta lo scraper di Arcaplanet e registra i job nella coda
 ...
 */
async function setupArcaplanetScraper(queue, proxy = null, interval = 3600000, options = {}) {
  logger.info('Setting up Arcaplanet scraper');
  
  // Merge default options per l'ambiente di produzione con approccio controllato
  const scraperOptions = {
    enablePagination: true,         // Abilita di default la paginazione
    maxPages: 5,                    // Imposta 5 pagine per categoria
    retryAttempts: 3,               // Numero di tentativi in caso di errore
    retryDelay: 5000,               // Pausa tra i tentativi (5 secondi)
    pauseBetweenCategories: 5000,   // Pausa tra categorie (5 secondi)
    pauseBetweenPages: 2000,        // Pausa tra pagine di una categoria (2 secondi)
    requestDelay: 3000,             // Pausa tra singole richieste API (3 secondi)
    ...options
  };
  
  logger.info(`Configurazione scraper Arcaplanet con approccio controllato: 
  ...
  // Registra il processor per i job Arcaplanet
  queue.process('scrape-arcaplanet', async (job) => {
    return await processArcaplanetJob(job.data, proxy, scraperOptions);
  });
  
  // Imposta un timer per il prossimo job
  queue.add('scrape-arcaplanet', { options: scraperOptions }, { 
    repeat: { 
      every: interval 
    },
  ...
  return await processArcaplanetJob({ options: scraperOptions }, proxy, scraperOptions);
}

/**
 * Elabora un job di scraping per Arcaplanet
 * @param {Object} jobData - Dati del job
 * @param {string|null} proxy - Proxy da utilizzare (opzionale)
 * @param {Object} options - Opzioni aggiuntive per lo scraper
 */
async function processArcaplanetJob(jobData, proxy = null, options = {}) {
  try {
    const jobId = jobData.id || 'manual';
    const startTime = new Date();
    
    logger.info(`Avvio job Arcaplanet #${jobId} - ${startTime.toISOString()}`);
    logger.info(`Opzioni scraper: ${JSON.stringify(options)}`);
    
    const results = await scrapeArcaplanet(proxy, options);
    
    const endTime = new Date();
    const duration = (endTime - startTime) / 1000;
    
    logger.info(`Completato job Arcaplanet #${jobId} in ${duration}s. Categorie processate: ${results.categoriesProcessed}, prodotti trovati: ${results.totalProducts}, salvati: ${results.savedProducts}, aggiornati: ${results.updatedProducts}`);
    ...
    logger.error(`Errore fatale nell'elaborazione del job Arcaplanet: ${error.message}`);
...
/**
 * Esegue lo scraping dei prodotti da Arcaplanet tramite GraphQL
 * @param {string|null} proxy - Proxy da utilizzare (opzionale)
 * @param {Object} options - Opzioni aggiuntive per lo scraper
 */
async function scrapeArcaplanet(proxy = null, options = {}) {
  // Merge default options con approccio controllato
  const scraperOptions = {
    enablePagination: true,
    maxPages: 5,
    retryAttempts: 3,
    retryDelay: 5000,
    pauseBetweenCategories: 5000,
    pauseBetweenPages: 2000,
    requestDelay: 3000,
    ...options
  };
  
  // Log delle opzioni configurate
  logger.info(`Configurazione scraper con approccio controllato:
    - Paginazione: ${scraperOptions.enablePagination ? 'abilitata' : 'disabilitata'}
    - Max pagine: ${scraperOptions.maxPages}
    - Tentativi: ${scraperOptions.retryAttempts}
    - Pausa tra categorie: ${scraperOptions.pauseBetweenCategories}ms
  ...
  logger.info(`Avvio scraping Arcaplanet per ${arcaplanetCategoryPaths.length} categorie via GraphQL`);
  ...
  for (const categoryPath of arcaplanetCategoryPaths) {
    ...
    for (let attempt = 1; attempt <= scraperOptions.retryAttempts; attempt++) {
      try {
        ...
        // Recupera i prodotti tramite API GraphQL
    ...
    if (categoryPath !== arcaplanetCategoryPaths[arcaplanetCategoryPaths.length - 1]) {
  ...
  logger.info(`Arcaplanet scraping completed in ${stats.executionTime.toFixed(2)} seconds.`);
...
async function saveProducts(products) {
  ...
  for (const product of products) {
    try {
      ...
      // Imposta sempre source a 'arcaplanet' se mancante
      if (!product.source) {
        product.source = 'arcaplanet';
      }
...
module.exports = {
  setupArcaplanetScraper,
  processArcaplanetJob,
  scrapeArcaplanet,
  saveProducts
};